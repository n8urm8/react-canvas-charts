...
import { renderChartGrid, type ChartGridProps } from './InternalComponents/ChartGrid'
...
export const ChartGridLayer: React.FC<ChartGridLayerProps> = ({
  ...
  const draw = useCallback<ChartLayerRenderer>(
    (context, helpers) => {
      const verticalLines =
        alignWithXAxisTicks && helpers.axisTicks.x ? helpers.axisTicks.x.positions : helpers.labelPositions
